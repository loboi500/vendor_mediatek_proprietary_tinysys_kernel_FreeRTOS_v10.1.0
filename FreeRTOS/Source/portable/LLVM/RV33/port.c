...
void vPortSetupTimer(void)
{
#if !defined(CFG_ESL_BUILD) && !defined(CFG_CSR_XGPT_SUPPORT)
    vic_enable();
    ...
    timer_disable();
    timer_clear_status();
#ifndef CFG_CSR_XGPT_SUPPORT
    timer_set_current_value(0x4000); /* write manually in this timer version */
    timer_set_reload_value(0x4000);
#else
    /* Configure SysTick to interrupt at the requested rate */
    uint32_t count_down = (configCPU_CLOCK_HZ / configTICK_RATE_HZ) - 1UL;
    timer_set_current_value(count_down); /* write manually in this timer version */
    timer_set_reload_value(count_down);
#endif
    timer_enable();
#endif
...
#ifdef APUSYS_TMR_WORKAROUND
    if (apusys_check_tmr_int() == 0)
        return;
#endif

#ifdef APUSYS_IRQ_WORKAROUND
    poll_apusys_irq();
#endif
