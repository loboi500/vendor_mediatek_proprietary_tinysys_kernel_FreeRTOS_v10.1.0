...
#if !defined(CFG_RV33_MCUPM) && !defined(CFG_RV33_GPUEB)
...
#define uxPortGetCoreId()	mrv_get_coreid()
#define uxPortGetHartId()	mrv_get_hartid()
/*-----------------------------------------------------------*/
...
#else
#define portENTER_CRITICAL()					{ if (!is_in_isr() && (xTaskGetSchedulerState() != taskSCHEDULER_NOT_STARTED)) vTaskEnterCritical(); }
#define portEXIT_CRITICAL()					{ if (!is_in_isr() && (xTaskGetSchedulerState() != taskSCHEDULER_NOT_STARTED)) vTaskExitCritical(); }
#endif
